...
CLIENTCONFSHDIR=$CONFDIR/client_sh
...
function incrementIP() {
    ...
    local fourth_part="${ip_parts[3]}"
    ((fourth_part++))

    echo "${ip_parts[0]}.${ip_parts[1]}.${ip_parts[2]}.$fourth_part"
}
...
function ipUsed() {
    ...
    ip_check_result=$(grep -c "$ip_to_check" "$CLIENTCONFDIR"/*)
...
function installUnbound() {
        ...
        if [[ ! -e /etc/unbound/unbound.conf ]]; then
                ...
                if [[ $OS =~ (debian|ubuntu) ]]; then
                        ...
                        # Configuration
                        echo 'interface: $class_c_ip.1
access-control: $class_c_ip.1/24 allow
hide-identity: yes
hide-version: yes
use-caps-for-id: yes
prefetch: yes' >>/etc/unbound/unbound.conf
                        ...
                        # Configuration
                        sed -i 's|# interface: 0.0.0.0$|interface: $class_c_ip.1|' /etc/unbound/unbound.conf
                        sed -i 's|# access-control: 127.0.0.0/8 allow|access-control: $class_c_ip.1/24 allow|' /etc/unbound/unbound.conf
...
function installOpenVPN() {
        ...
        # Install the templates dir, if not already installed.
        if [[ ! -d /etc/openvpn/templates/ ]]; then
                mkdir -p /etc/openvpn/templates
                touch /etc/openvpn/templates/ccd.tmpl
                echo "ifconfig-push __CLIENTIP__ 255.255.255.0" >> /etc/openvpn/templates/ccd.tmpl
                echo 'push "route $class_c_ip.0 255.255.254.0"' >> /etc/openvpn/templates/ccd.tmpl
...
keepalive 10 120
topology subnet
server $class_c_ip.0 255.255.255.0" >>/etc/openvpn/server.conf
        ...
        2) # Self-hosted DNS resolver (Unbound)
                echo 'push "dhcp-option DNS $class_c_ip.1"' >>/etc/openvpn/server.conf
                if [[ $IPV6_SUPPORT == 'y' ]]; then
...
verb 3" >>/etc/openvpn/server.conf
        ...
        # Create client-config-sh-dir dir
        mkdir -p "$CLIENTCONFSHDIR"
        # Create log dir
        ...
        # Script to add rules
        echo "#!/bin/sh
iptables -t nat -I POSTROUTING 1 -s $class_c_ip.0/24 -o $NIC -j MASQUERADE
...
ip6tables -I INPUT 1 -i $NIC -p $PROTOCOL --dport $PORT -j ACCEPT" >>/etc/iptables/add-openvpn-rules.sh
        ...
        echo "#!/bin/sh
iptables -t nat -D POSTROUTING -s $class_c_ip.0/24 -o $NIC -j MASQUERADE
...
# Check to see if Configuration Files for Hostname already exists
if [ -f ${CLIENTCONFDIR}/${CLIENT} ]; then
   echo -e "An OpenVPN client configuration already exists for $CLIENT:\n"
   ls -lt ${CLIENTCONFDIR}/${CLIENT}
   exit 2;
else
        echo ""
...
# Initialize a variable to track the next available IP
NEXT_IP="${start_ip}"

# Check each IP address within the range
while [ "${NEXT_IP}" != "${end_ip}" ]; do
    # Check if the IP is in use by looking for it in existing files
    if ! grep -q "${NEXT_IP}" "${CLIENTCONFDIR}"/*; then
        # If not in use, break the loop
        break
    fi

    # Increment the IP for the next check
    IP_PARTS=($(echo "${NEXT_IP}" | tr '.' ' '))
    # Increment the fourth part of the IP
    IP_PARTS[3]=$((IP_PARTS[3] + 1))

    # If the fourth part exceeds 253, increment the third part by 1 and reset the fourth part to 1
    if [ "${IP_PARTS[3]}" -gt 253 ]; then
        IP_PARTS[2]=$((IP_PARTS[2] + 1))
        IP_PARTS[3]=1
    fi

    # Reconstruct the next IP
    NEXT_IP="${IP_PARTS[0]}.${IP_PARTS[1]}.${IP_PARTS[2]}.${IP_PARTS[3]}"
done

# Use the available IP as CLIENTIP
CLIENTIP="${NEXT_IP}"



echo "Assiging $CLIENTIP and $SERVERIP to tunnel..." | tee -a $CONNECTION_LOG
...
cat ${CONFDIR}/templates/ccd.tmpl | sed "s/__CLIENTIP__/${CLIENTIP}/;s/__SERVERIP__/${SERVERIP}/;s/__REMOTESERVER__/$REMOTESERVER/" > ${CLIENTCONFDIR}/${CLIENT}
...
function revokeClient() {
        ...
        EASYRSA_CRL_DAYS=3650 ./easyrsa gen-crl
        rm -f ${CONFDIR}/crl.pem
        cp ${EASYRSA}/pki/crl.pem /etc/openvpn/crl.pem
        chmod 644 /etc/openvpn/crl.pem
        find /home/ -maxdepth 2 -name "$CLIENT.ovpn" -delete
        find /root/ -maxdepth 2 -name "$CLIENT.ovpn" -delete
        rm -f "/root/$CLIENT.ovpn"

        cp /etc/openvpn/easy-rsa/pki/index.txt{,.bk}
        rm -f "$REVOKEDCRTSER/$CLIENTNUMBER"

        sed --in-place "/$CLIENT/d" /etc/openvpn/easy-rsa/pki/index.txt

        rm -f "$CLIENTCONFDIR/$CLIENT"
        rm -f "$CLIENTCONFSHDIR/$CLIENT.sh"


        echo ""
        echo "Certificate for client $CLIENT revoked."
        echo -e "OpenVPN Client Configuration for ${CLIENT} has been successfully removed on ${GENDATE}\n" | tee -a $DELETE_LOG
...
function manageMenu() {
    ...
    echo "   *) Remove OpenVPN"
    echo "   4) Exit"
    until [[ $MENU_OPTION =~ ^[124*]$ ]]; do
        read -rp "Select an option [1, 2, *, 4]: " MENU_OPTION
    done
